...
namespace WebStore.Clients
{
    public class ValuesClient : BaseClient, IValuesService
    {
        ...
        {
            var response = await
            Client.PostAsJsonAsync($"{ServiceAddress}", value);
            response.EnsureSuccessStatusCode();
            return response.StatusCode;
        }
        public HttpStatusCode Put(int id, string value)
        {
            var response = Client.PutAsJsonAsync($"{ServiceAddress}/{id}",
            value).Result;
            response.EnsureSuccessStatusCode();
            return response.StatusCode;
        }
        ...
        {
            var response = await
            Client.PutAsJsonAsync($"{ServiceAddress}/{id}", value);
            response.EnsureSuccessStatusCode();
            return response.StatusCode;
        }
        ...
        {
            var response =
            Client.DeleteAsync($"{ServiceAddress}/{id}").Result;
            return response.StatusCode;
        }
        public async Task<HttpStatusCode> DeleteAsync(int id)
        {
            var response = await
            Client.DeleteAsync($"{ServiceAddress}/{id}");
            return response.StatusCode;
        }
    }
